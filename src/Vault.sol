...
contract Vault is IERC4626, ERC20Permit, ILiquidationSource, IClaimable, Ownable {
  ...
  /**
   * @notice Emitted when a permit call on the underlying asset failed to increase the spending allowance.
   ...
   */
  error PermitAllowanceDidNotIncrease(address owner, address spender, uint256 amount);
  ...
  function depositWithPermit(
    ...
    uint256 allowanceBefore = _asset.allowance(_owner, address(this));
    IERC20Permit(address(_asset)).permit(_owner, address(this), _assets, _deadline, _v, _r, _s);
    if (_asset.allowance(_owner, address(this)) <= allowanceBefore) {
      revert PermitAllowanceDidNotIncrease(_owner, address(this), _assets);
    }
  ...
  function withdraw(
    ...
    uint256 _shares = _previewWithdraw(_assets);
  ...
  /**
   * @notice Returns the amount of shares that would be burned to receive the given assets.
   * @param _assets Amount of assets to withdraw
   * @return uint256 Amount of shares to be burned
   */
  ...
  /**
   * @notice Returns the amount of assets that would be received for the redemption of the given shares.
   * @param _shares Amount of shares to burn
   * @return uint256 Amount of assets to be received
   */
  ...
  /* ============ Asset Redemption Function ============ */

  /**
   * @notice Redeem/Withdraw common flow
   * @dev Burns shares and withdraws assets from the underlying yield vault.
